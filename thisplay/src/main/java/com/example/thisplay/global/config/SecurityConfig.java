    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/login", "/", "/join", "/logout", "/api/main/genres/**","/api/movies/show/**","/oauth2/**", "/login/oauth2/**","/api/likes/**","/api/users/*/profile").permitAll() //인증 없이 접근 가능
                        .requestMatchers("api/reviews/**", "/api/likes/**").authenticated()
                        .requestMatchers(HttpMethod.PATCH,"/api/users/*/profile").authenticated()
                        .requestMatchers(HttpMethod.GET, "/api/reviews/**", "/api/likes/**", "/api/main/genres/**", "/api/movies/show/**"
                        ).permitAll()
