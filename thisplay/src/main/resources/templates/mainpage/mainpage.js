...
// import { isLoggedIn } from "../../static/js/utils/auth.js";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadGenres() {
    try {
      const res = await fetch(`${BASE_URL}/api/main/genres`);
  ...
  async function loadMovies(genreId) {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/main/genres/${genreId}/top20`);
      if (!res.ok) throw new Error(`HTTP 오류: ${res.status}`);
